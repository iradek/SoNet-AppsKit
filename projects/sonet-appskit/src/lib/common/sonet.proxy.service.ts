...
export class SoNetProxy extends HttpClient {
     ...
     * @param api use if there is need to send request to different back-end than the default one.
     */
    ...
    /**
     * PATCH request with resolving relative Url of an endpoint.
     * @param endPoint relative location of method to call
     * @param params body of the request.
     * @param options options of the request like headers, body, etc.
     * @param api use if there is need to send request to different back-end than the default one.
     */
    public patch$<T>(endPoint: string, params: object | string, options?: IRequestOptions, api?: string): Observable<T> {
        return super.patch<T>(api || this.urlService.resolveFinalUrl(endPoint), params, options);
    }
